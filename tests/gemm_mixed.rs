...
#[test]
fn q8_0_mlp_gate_up_coltile4_split4_matches_baseline() -> Result<()> {
    let ctx = match cuda_env::ctx_m40_or_skip() {
        Some(ctx) => ctx,
        None => return Ok(()),
    };
    ...
    unsafe {
        ...
        {
            let _split = EnvRestore::set("M40LLM_Q8_MLP_GATE_UP_SPLIT_QBLOCK", "4");
            let _tile = EnvRestore::set("M40LLM_Q8_MLP_GATE_UP_COL_TILE", "4");
            ctx.mlp_gate_up_swiglu_f32xq8_0_gguf_decode_async(
                da as *const c_void,
                d_gate as *const c_void,
                d_up as *const c_void,
    ...
    assert!(
        max_diff <= 1e-4 && mean_diff <= 1e-5,
        "q8 coltile4 split4 MLP mismatch max_diff={max_diff} mean_diff={mean_diff}"
    );
