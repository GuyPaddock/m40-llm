...
#[test]
fn attention_last_token_cuda_gqa_head128_group8_qwen_shape_parity() -> Result<()> {
    struct EnvGuard {
        key: &'static str,
        previous: Option<String>,
    }
    ...
    let _kernel = EnvGuard::set("M40LLM_ATTN_HEAD128_KERNEL", "group8");
    let ctx = match cuda_env::ctx_m40_or_skip() {
        Some(ctx) => ctx,
        None => return Ok(()),
    };
    ...
    for &seq_len in &seq_lens {
        ...
        for i in 0..q_dim {
            ...
            assert!(
                diff < 1e-3,
                "Qwen-shaped group8 head128 mismatch (seq_len={}, i={}) cpu={} gpu={} diff={}",
                seq_len,
                i,
                out_cpu[i],
                out_gpu[i],
