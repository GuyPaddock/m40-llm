...
#[test]
fn gemm_f16xf16_f32_square_2x2() -> Result<()> {
    ...
    let a_half = f32s_to_halves_bytes(&a);
    ...
    unsafe {
        ctx.memcpy_h2d(da, a_half.as_ptr() as *const c_void, bytes_a)?;
...
#[test]
fn gemm_f16xf16_f32_rectangular_1x3x2() -> Result<()> {
    let ctx = match cuda_env::ctx_m40_or_skip() {
        Some(ctx) => ctx,
        None => return Ok(()),
    };
