...
pub fn load_gguf(path: &Path) -> Result<GgufModel> {
    let mut f = File::open(path).with_context(|| format!("failed to open GGUF file {:?}", path))?;
    ...
    for _ in 0..n_kv {
        ...
        let val = match vt {
            GgufValueType::Array => {
                ...
                let elem_type = GgufValueType::from_u32(elem_type_raw).ok_or_else(|| {
                    anyhow::anyhow!("unknown GGUF array element type: {}", elem_type_raw)
                })?;
...
fn read_string<R: Read>(r: &mut R) -> Result<String> {
    ...
    r.read_exact(&mut buf)?;
    let s = String::from_utf8(buf).context("GGUF string is not valid UTF-8")?;
    Ok(s)
}
