...
| Qwen2.5-3B Q8_0 | large-model | dense off | previous row + head128 probability reuse in attention V loop | 44 | 512 | 1232 ms | 21004 ms | 22418 ms | 24.38 | 22.84 | coherent |
...
| `M40LLM_ATTN_HEAD128_KERNEL=group8` grouped Qwen GQA attention | preservation commit | follow-up removal commit | Qwen-shaped parity passed for `q_heads=16`, `kv_heads=2`, `head_dim=128`, and seq_lens through 1024, but the exact 512-token Qwen2.5 Q8_0 target regressed to `prefill_ms=1255 decode_ms=22756 total_ms=24205 decode_tps=22.50 total_tps=21.15` | Slower than the default head128 probability-reuse result: `22418 ms`, `22.84 E2E tok/s` | Remove from active code after preserving this failed experiment; grouping all 8 Q heads per KV head increases per-block work/register pressure enough to lose on M40 |
