...
| `M40LLM_ATTN_HEAD128_KERNEL=group4` grouped Qwen GQA attention | preservation commit | follow-up removal commit | Qwen-shaped parity passed for `q_heads=16`, `kv_heads=2`, `head_dim=128`, and seq_lens through 1024, but the exact 512-token Qwen2.5 Q8_0 target regressed to `prefill_ms=1131 decode_ms=20858 total_ms=22171 decode_tps=24.55 total_tps=23.09` | Slower than the current default split-Q-block/probability-reuse result: `21030 ms`, `24.35 E2E tok/s` | Remove from active code after preserving this failed experiment; grouping four Q heads per block still loses enough occupancy/register efficiency to offset K/V reuse on M40 |
